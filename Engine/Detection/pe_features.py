...
class SectionInfo(FeatureType):
    ...
    def raw_features(self, bytez, lief_binary):
        ...
        try:
            if int(LIEF_MAJOR) > 0 or (int(LIEF_MAJOR) == 0 and int(LIEF_MINOR) >= 12):
                ...
                section = lief_binary.section_from_offset(lief_binary.entrypoint)
        except Exception:
            section = None
    ...
    def process_raw_features(self, raw_obj):
        ...
        # EMBER passed the bare entry-name string as a single sample; older sklearn
        # iterated it into individual characters and hashed each one. sklearn >=1.2
        # rejects a bare string sample, so pass the characters explicitly to
        # reproduce EMBER's exact hashed values (same 50-dim layout).
...
class PEFeatureExtractor(object):
    ...
    def raw_features(self, bytez):
        # Modern LIEF (0.14+) returns None on failure instead of raising, and the
        # old lief.bad_format / lief.pe_error exception symbols no longer exist, so
        # catch broadly and treat any failure as None. NOTE: passing raw `bytes` to
        # lief.PE.parse is interpreted as a *filename*; wrap in io.BytesIO so LIEF
        # parses the buffer contents.
        try:
            lief_binary = lief.PE.parse(io.BytesIO(bytez))
        except Exception:
            lief_binary = None
    ...
    def _feature_vector_impl(self, bytez):
        return self.process_raw_features(self.raw_features(bytez))

    def feature_vector(self, bytez):
        ''' Return a fixed 2381-dim float32 vector. Never raises on malformed
        input: returns a zero vector of the correct dimension instead. '''
        try:
