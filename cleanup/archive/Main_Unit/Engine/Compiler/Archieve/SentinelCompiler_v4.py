...
from Main_Unit.Units.Archieve.Engine_Unit_ML import model_scanner
...
class VirusScanner:
    ...
    def scan_file(self, file_path):
        ...
        # Final verdict logic
        # Note: The special ML exception has already forced MALWARE above when applicable
        if layer_hits >= 2:
            verdict = "MALWARE"
        elif layer_hits == 1 and verdict != "MALWARE":
            # If only a single non-exception layer hit, mark as suspicious
            verdict = "SUSPICIOUS"
        ...
        if verdict == "MALWARE":
            ...
            toast.show()
            #save_suspect_path(file_path)
            #save_suspect_file_original_destination_path(file_path)
    ...
    def scan_directory(self, target_dir, notify_confirmed_only=True,
        ...
        """
        Scan directory focusing on PE files only.
        Returns: (scan_count: int, detections: list[dict])
        """
        if not os.path.isdir(target_dir):
            raise ValueError(f"'{target_dir}' is not a directory")

        print(f"\n🛡️ PE-Focused Scan: {os.path.abspath(target_dir)}")
        scan_count = 0
        detections = []
        confirmed_malware = []

        # ✅ executor passed from monitor.py (has QApplication)
        if executor is None:
            print("⚠️ No executor provided - threats logged only")

        for root, dirs, files in os.walk(target_dir):
            if running_flag is not None and not running_flag[0]:  # running_flag is [bool]
                print("⏹️ Scan cancelled by running_flag")
                break

            for file in files:

                # Check flags BEFORE scanning each file
                if cancel_flag is not None and cancel_flag[0]:
                    print("⏹️ Scan cancelled by cancel_flag")
                    return scan_count, detections
                if running_flag is not None and not running_flag[0]:
                    print("⏹️ Scan cancelled by running_flag")
                    return scan_count, detections

                file_path = os.path.join(root, file)
                if self.should_scan_file(file_path):  # PE extension + MZ/PE verification
                    try:
                        ...
                        if result['verdict'] != "CLEAN":
                            ...
                            if result['verdict'] == "MALWARE" and notify_confirmed_only:
                                ...
                                # 🚨 Queue confirmed malware (only if executor available)
                                if executor:
                                    task_id = executor.handle_threat(file_path)
                                    print(f"Task {task_id} added to queue for: {file_path}")
                                else:
                                    print(f"🚨 MALWARE DETECTED (no executor): {file_path}")
                        scan_count += 1
                    except Exception as e:
        ...
        print(
            f"✅ Analyzed {scan_count} PE files ({len(detections)} detections, "
